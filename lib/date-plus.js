...
module.exports = {
    activate: function(state) {
        atom.commands.add('atom-text-editor', {
            'date-plus:date': function(event) {
                var editor = this.getModel();
                insert(get('date'), editor);
            },
            'date-plus:time': function(event) {
                var editor = this.getModel();
                insert(get('time'), editor);
            },
            'date-plus:datetime': function(event) {
                var date = get('date');
                var time = get('time');
                var editor = this.getModel();
...
function insert(text, editor) {
    var prePadding = atom.config.get('date-plus.prePadding')
        .replace(/\\r\\n|\\n|\\r/g, "\r\n");
    var postPadding = atom.config.get('date-plus.postPadding')
        .replace(/\\r\\n|\\n|\\r/g, "\r\n");

    if (atom.config.get("date-plus.prePaddingMatchLineLength")) {
        editor.insertText(createPadding(prePadding.charAt(0), text.length) + "\r\n");
    } else {
        editor.insertText(prePadding);
    }
    editor.insertText(text);
    if (atom.config.get("date-plus.postPaddingMatchLineLength")) {
...
function get(format) {
    return new Date().toFormat(atom.config.get('date-plus.' + format + 'Format'));
}
